...
from backend.ml.sentiment.data_process import normalize_string, get_dictionary
...
DATA_PATH = os.path.join(ASSETS_DIR, "train.tsv")
...
def _load_model():
    ...
    with _lock:
        ...
        # 构建词典
        _, word2index, index2word = get_dictionary(DATA_PATH)
        _word2index = word2index
        _index2word = index2word
...
def predict(text: str):
    ...
    # 提取有效词的注意力权重（去掉填充部分）
    attn = attn_weights.squeeze(0).squeeze(1).cpu().tolist()  # [seq_len]
    ...
    if attn:
        ...
        min_a = min(attn)
        if max_a > min_a:
            attn = [(a - min_a) / (max_a - min_a) for a in attn]
    ...
    return {
        ...
        "words": words,
        "attn_weights": [round(a, 4) for a in attn]
    }


def is_ready() -> bool:
    """检查模型是否已就绪"""
    return os.path.exists(MODEL_PATH) and os.path.exists(DATA_PATH)
