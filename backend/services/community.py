...
class CommunityService(ICommunityService):
    ...
    def get_post_detail_service(self, post_id) -> ApiResponse[PostVO]:
        """获取帖子详情"""
    ...
    def update_post_service(self, post_id, update_data, user) -> ApiResponse:
        """编辑帖子"""
        post = community_db.get_post_by_id(post_id)
        if not post:
            raise BusinessException("帖子不存在", code=404)
    ...
    def delete_post_service(self, post_id, user) -> ApiResponse:
        """删除帖子"""
        post = community_db.get_post_by_id(post_id)
        if not post:
            raise BusinessException("帖子不存在", code=404)
    ...
    def get_comments_service(self, post_id, page=1, page_size=20) -> ApiResponse[list[CommentVO]]:
        """获取帖子的评论列表"""
    ...
    def create_comment_service(self, post_id, content, parent_id, is_anonymous, user) -> ApiResponse:
        """发表评论"""
    ...
    def delete_comment_service(self, comment_id, user) -> ApiResponse:
        """删除评论（作者本人或管理员）"""
    ...
    def toggle_like_service(self, post_id, user) -> ApiResponse[LikedVO]:
        """点赞/取消点赞"""
    ...
    def has_liked_service(self, post_id, user) -> ApiResponse[LikedVO]:
        """检查当前用户是否已点赞"""
    ...
    def mark_read_service(self, notif_id, user) -> ApiResponse:
        """标记单条已读"""
    ...
    def delete_notification_service(self, notif_id, user) -> ApiResponse:
        """删除通知（只能删自己的）"""
