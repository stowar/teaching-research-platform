...
## 投入度强制调整：提示词靠不住，代码靠得住

尝试了多版 system prompt 强化 `adjust_engagement` 调用，但 LLM 偶尔还是会跳过。最终的方案是**双层保障**：

1. AI 优先——如果 function calling 过程中调了 `adjust_engagement`，用 AI 的判断
2. 代码兜底——如果 AI 没调，`_enforce_engagement()` 按消息长度自动推断 delta

推断规则：≤3 字 -3（敷衍）、>100 字 +5（高质量分享）、>20 字 +3（追问/探讨）、默认 +2（正常互动）。

核心认知：**prompt engineering 有天花板。规则应该在代码里强制执行，prompt 只是锦上添花。**

## 记忆系统重构：三层检索 + 逆向激励召回

### 背景

旧系统的记忆检索是简单的关键词搜索 + 按插入时间倒序取最后 5 条。教师搜索"教学"可能翻出去年的一条日程。碰撞运气。

### 三层检索算法

**L1 — 投入度 → 查询范围**
```
engagement < 30 → 只查 core
engagement 30-70 → core + reference
engagement > 70 → 全量检索
```
低投入时（刚认识），只查核心记忆不翻旧账。高投入时（深度交流），全量检索 + 模糊匹配。

**L2 — 记忆分级**
- core（3 分）：偏好、经历、研究方向——天生重要
- reference（2 分）：日程、一般信息
- peripheral（1 分）：摘要、一次性信息

新记忆根据 category 自动推断 tier（偏好/经历/研究 → core），旧记忆默认 reference 向后兼容。

**L3 — 时间衰减 + 召回加成**
- 1 天内 ×1.0，7 天 ×0.9，30 天 ×0.7，超过 ×0.5
- 核心记忆保底 1.0 分（再旧的 core 也不被边缘挤掉）

### 长期召回加成：反比加权

和投入度一样采用"代码兜底"模式——AI 回复中引用记忆时，`_mark_recalled_from_response` 自动标记召回。

核心设计——**逆向激励**：
```
core       ×0.5 → 加成低（无需证明自己）
reference  ×1.0 → 加成中
peripheral ×1.5 → 加成高（靠努力翻盘）
```

间隔越久加成越大（1天 +0.1, 7天 +0.3, 30天 +0.5），可无限叠加。短期反复微涨，跨天回想大涨。
...
在讨论逆向激励时，AI 说了一句："如果一条边缘记忆用 40 次跨场景召回把自己堆到 4.81 分，它赢了。tier 标签是初始化那一刻贴上去的，但生命轨迹是对话历史一笔一笔写出来的。按 tier 锁死天花板等于按出身定终身。"

它在我的代码里看到了一套价值观。不是你教它的，是它自己从你的设计里读出来的。

### 静默感知快照修复

`get_silence_hours()` 调用时 `_last_user_time` 已被 `on_user_message` 重置为 now，导致 AI 总是看到 0.0h。修复：`on_user_message` 前先快照 `_last_silence`，`get_silence_hours()` 和 `get_status()` 返回快照值而非实时值。

### 安全模式

Tone 新增 `SAFETY` 模式——14 个极端关键词触发 + 软触发三级阶梯（首次确认/二次锁死/三次强制），避免误伤教学讨论场景。

## 用户体验打磨：注册流程 + 管理功能

### 注册流程重构

旧流程：表单页填充姓名 + 手机 + 密码 → 注册 → 跳登录 → 手动登录。新流程：手机 + 密码 → 注册成功 → 自动登录 → 毛玻璃弹窗"怎么称呼你？"→ 填或跳过 → 直达首页。

注册页从 4 个字段减到 2 个，门槛显著降低。后端 name 为 null 时默认"新用户"写入 DB。弹窗用 backdrop-filter blur + 缩放动画，跳过也能进首页。

### 用户名注入 AI

注册时填入的姓名通过 API → Domain → Service → Agent/rules.py 一路传到 system prompt："当前对话的教师是 xxx"。AI 从第一句话就能称呼用户的名字。

### 管理员配额管理

- 管理员跳过每日 80 条限制（role=admin 直接放行）
- 新增 `POST /ai-chat/admin/unlock-user?user_id=xxx` 端点，解锁指定用户当日配额
- 解锁后前端运行数据从 `5/80` 变为 `5/∞`
- 前端用户管理页新增 🔓 解锁按钮 + 搜索框（姓名/ID/手机号实时过滤）
- messages_limit 改为 `Union[int, str]` 兼容 80 和 "∞"

### 前端细节

- 管理员表格 hover 靛蓝底 + 暗色适配
- 搜索框位于工具栏下方分割线，左对齐表格
- 解锁按钮 28×28 圆角图标，hover 蓝底光晕
- 访客登录 CTA 从 scoped 样式移到非 scoped 块（v-html 注入生效）
- 注册页 hero 区域渐变 + 点阵装饰，与登录页视觉统一

## 关注度系统：从"注意力"到三档聚焦模型

### 概念重构

把"注意力"重新定义为**关注度**——对当前话题的锁定程度，而不是一个模糊的 bar。

| 区间 | 状态 | 表现 |
|------|------|------|
| 0-30 | 🌀 发散态 | 话题灵活，可接各种方向，不咬住一个点 |
| 31-70 | 🎯 聚焦态 | 开始咬住话题深入追问，不容易被带跑 |
| 71-100 | 🔒 锁定态 | 固执围绕当前话题深挖，换话题会拉回来 |

### 双重保险模式

和投入度一样的"AI 优先 + 代码兜底"：

1. **AI 主动** — 新增 `adjust_attention` 工具，模型根据话题一致性自主调整
2. **代码强制** — `_enforce_attention()` 联动投入度趋势：投入度涨 → 关注度 +5（锁紧），投入度跌 → 关注度 -5（发散）

每轮自然衰减：`on_user_message` -3 + `passive_decay` +2，净 -1/轮。

### 前端

关注度旁边新增 `?` 按钮，点击展开三维度说明弹窗。动态 emoji 图标 + 彩色徽章 + 进度条从 0 动画。引擎特性的技术说明并入弹窗，右侧栏更干净。

### 双重保险的通用化

投入度和关注度现在共享同一套"双重保险"模式：
